...
type Payment struct {
	...
	UserPaymentMethodID int64     `boil:"user_payment_method_id" json:"user_payment_method_id" toml:"user_payment_method_id" yaml:"user_payment_method_id"`
...
var PaymentColumns = struct {
	...
	ID:                  "id",
	UserPaymentMethodID: "user_payment_method_id",
...
var PaymentTableColumns = struct {
	...
	ID:                  "payments.id",
	UserPaymentMethodID: "payments.user_payment_method_id",
	CurrencyID:          "payments.currency_id",
...
var PaymentWhere = struct {
	ID                  whereHelperint64
	UserPaymentMethodID whereHelperint64
	...
	UserPaymentMethodID: whereHelperint64{field: "\"payments\".\"user_payment_method_id\""},
	CurrencyID:          whereHelperint64{field: "\"payments\".\"currency_id\""},
	Amount:              whereHelperint{field: "\"payments\".\"amount\""},
...
type paymentR struct {
	...
	UserPaymentMethod *UserPaymentMethod `boil:"UserPaymentMethod" json:"UserPaymentMethod" toml:"UserPaymentMethod" yaml:"UserPaymentMethod"`
...
func (r *paymentR) GetCurrency() *Currency {
	if r == nil {
		return nil
	}
	return r.Currency
...
func (r *paymentR) GetUserPaymentMethod() *UserPaymentMethod {
	if r == nil {
		return nil
	}
	return r.UserPaymentMethod
}
...
var (
	paymentAllColumns            = []string{"id", "user_payment_method_id", "currency_id", "amount", "updated_at", "created_at"}
	paymentColumnsWithoutDefault = []string{"user_payment_method_id", "currency_id", "amount"}
	paymentColumnsWithDefault    = []string{"id", "updated_at", "created_at"}
...
func (o *Payment) Currency(mods ...qm.QueryMod) currencyQuery {
...
// UserPaymentMethod pointed to by the foreign key.
func (o *Payment) UserPaymentMethod(mods ...qm.QueryMod) userPaymentMethodQuery {
	queryMods := []qm.QueryMod{
		qm.Where("\"id\" = ?", o.UserPaymentMethodID),
	}
...
func (paymentL) LoadCurrency(ctx context.Context, e boil.ContextExecutor, singular bool, maybePayment interface{}, mods queries.Applicator) error {
	var slice []*Payment
	var object *Payment
...
// LoadUserPaymentMethod allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for an N-1 relationship.
func (paymentL) LoadUserPaymentMethod(ctx context.Context, e boil.ContextExecutor, singular bool, maybePayment interface{}, mods queries.Applicator) error {
	var slice []*Payment
	var object *Payment
	...
	if singular {
		...
		args = append(args, object.UserPaymentMethodID)
	...
	Outer:
		for _, obj := range slice {
			...
			for _, a := range args {
				if a == obj.UserPaymentMethodID {
					continue Outer
				}
			}

			args = append(args, obj.UserPaymentMethodID)
	...
	query := NewQuery(
		...
		qm.WhereIn(`user_payment_methods.id in ?`, args...),
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load UserPaymentMethod")
	}

	var resultSlice []*UserPaymentMethod
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice UserPaymentMethod")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results of eager load for user_payment_methods")
	}
	if err = results.Err(); err != nil {
	...
	if singular {
		foreign := resultSlice[0]
		object.R.UserPaymentMethod = foreign
		if foreign.R == nil {
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.UserPaymentMethodID == foreign.ID {
				local.R.UserPaymentMethod = foreign
				if foreign.R == nil {
					foreign.R = &userPaymentMethodR{}
				}
				foreign.R.Payments = append(foreign.R.Payments, local)
...
func (paymentL) LoadOrders(ctx context.Context, e boil.ContextExecutor, singular bool, maybePayment interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`orders`),
		qm.WhereIn(`orders.payment_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// SetCurrency of the payment to the related item.
// Sets o.R.Currency to related.
// Adds o to related.R.Payments.
func (o *Payment) SetCurrency(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Currency) error {
	var err error
	if insert {
...
// SetUserPaymentMethod of the payment to the related item.
// Sets o.R.UserPaymentMethod to related.
// Adds o to related.R.Payments.
func (o *Payment) SetUserPaymentMethod(ctx context.Context, exec boil.ContextExecutor, insert bool, related *UserPaymentMethod) error {
	var err error
	if insert {
		if err = related.Insert(ctx, exec, boil.Infer()); err != nil {
			return errors.Wrap(err, "failed to insert into foreign table")
	...
	o.UserPaymentMethodID = related.ID
	if o.R == nil {
		o.R = &paymentR{
			UserPaymentMethod: related,
		}
	} else {
		o.R.UserPaymentMethod = related
	}

	if related.R == nil {
		related.R = &userPaymentMethodR{
			Payments: PaymentSlice{o},
		}
	} else {
		related.R.Payments = append(related.R.Payments, o)
...
// Sets o.R.Payment's Orders accordingly.
// Replaces o.R.Orders with related.
// Sets related.R.Payment's Orders accordingly.
func (o *Payment) SetOrders(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Order) error {
	query := "update \"orders\" set \"payment_id\" = null where \"payment_id\" = $1"
...
func (o *Payment) RemoveOrders(ctx context.Context, exec boil.ContextExecutor, related ...*Order) error {
	if len(related) == 0 {
		return nil
	}
