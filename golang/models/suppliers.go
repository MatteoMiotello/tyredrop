...
func (o *Supplier) OrderRows(mods ...qm.QueryMod) orderRowQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"order_rows\".\"supplier_id\"=?", o.ID),
	)
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (supplierL) LoadOrderRows(ctx context.Context, e boil.ContextExecutor, singular bool, maybeSupplier interface{}, mods queries.Applicator) error {
	var slice []*Supplier
	var object *Supplier
	...
	query := NewQuery(
		qm.From(`order_rows`),
		qm.WhereIn(`order_rows.supplier_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// Sets related.R.Supplier appropriately.
func (o *Supplier) AddOrderRows(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*OrderRow) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.SupplierID = o.ID
