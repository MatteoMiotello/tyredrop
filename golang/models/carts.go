...
var CartTableColumns = struct {
	...
	ProductItemPriceID: "carts.product_item_price_id",
}
...
var CartWhere = struct {
	...
	ProductItemPriceID: whereHelperint64{field: "\"carts\".\"product_item_price_id\""},
}
...
func (r *cartR) GetProductItemPrice() *ProductItemPrice {
	if r == nil {
		return nil
	}
...
var (
	cartAllColumns            = []string{"id", "user_id", "quantity", "deleted_at", "updated_at", "created_at", "product_item_price_id"}
	cartColumnsWithoutDefault = []string{"user_id", "quantity", "product_item_price_id"}
	cartColumnsWithDefault    = []string{"id", "deleted_at", "updated_at", "created_at"}
...
func (o *Cart) ProductItemPrice(mods ...qm.QueryMod) productItemPriceQuery {
...
func (cartL) LoadProductItemPrice(ctx context.Context, e boil.ContextExecutor, singular bool, maybeCart interface{}, mods queries.Applicator) error {
	var slice []*Cart
	var object *Cart
...
// SetProductItemPrice of the cart to the related item.
// Sets o.R.ProductItemPrice to related.
// Adds o to related.R.Carts.
func (o *Cart) SetProductItemPrice(ctx context.Context, exec boil.ContextExecutor, insert bool, related *ProductItemPrice) error {
	var err error
	if insert {
...
func Carts(mods ...qm.QueryMod) cartQuery {
	mods = append(mods, qm.From("\"carts\""), qmhelper.WhereIsNull("\"carts\".\"deleted_at\""))
	q := NewQuery(mods...)
...
func FindCart(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*Cart, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"carts\" where \"id\"=$1 and \"deleted_at\" is null", sel,
	)
...
func (o *Cart) Delete(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if o == nil {
	...
	if hardDelete {
		...
		sql = fmt.Sprintf("UPDATE \"carts\" SET %s WHERE \"id\"=$2",
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
		valueMapping, err := queries.BindMapping(cartType, cartMapping, append(wl, cartPrimaryKeyColumns...))
		if err != nil {
			return 0, err
		}
...
func (q cartQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if q.Query == nil {
...
func (o CartSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if len(o) == 0 {
		return 0, nil
	}
	...
	if hardDelete {
		...
		wl := []string{"deleted_at"}
		sql = fmt.Sprintf("UPDATE \"carts\" SET %s WHERE "+
			strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 2, cartPrimaryKeyColumns, len(o)),
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
...
func CartExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"carts\" where \"id\"=$1 and \"deleted_at\" is null limit 1)"
